...
int EnemyAttack(Player* player, Enemy* enemy)
{
    ...
    PrintSlowly("Враг атакует! Оставшееся здоровье: " + to_string(player->GetCharacter()->GetHealth()) + "\n", false);
    Pause(false);
    return damage;
}
...
{
    enemy->GetInfo();
    PrintSlowly("Ваше здоровье: " + to_string(player->GetCharacter()->GetHealth()) + " HP\n\n", false);
    while (true)
    {
...
int main()
{
    ...
    PrintSlowly("Вдалеке я увидел старый разрушенный город и огромные величественные ворота! за которыми... в стальных оковах ... жил защитник этого разрушенного города - Китава", false);
    ...
    cout << "\n\t 1 - посетить пещеру \n\t 2 - отправиться в разрушенный город" << endl;
    ...
    cout << "\n\t 1 - " + name + " берет сердце в руки\n\t 2 - " + name + " заходит в ворота Китавы " << endl;
